...
"""PyDAQmx provides an OOP wrapper to the DAQmx C Library. All commands parallel their
C equivalent one to one. For documentation on the functionality of each DAQmx command,
see
...
def ZeroOutput():
    """Utility function that sets all the analog output channels of the DAQ card to 0V"""
    ...
    task.WriteAnalogF64(2,
                        ...
                        np.array([0.0],dtype=np.float64), # pylint: disable=E1101
                        wrote,
                        None
    )
...
class AnalogTask(Task):
    ...
    def __init__(self, lamb, amplitude):
        ...
        self.write = lamb/(t)
        self.write[np.where(self.write>amplitude)] = amplitude # Modulate the amplitude
        self.write = np.append(self.write, [self.write[0]]) # Write a zero at the end of 
...
class CompensationTask(Task):
    ...
    def __init__(self, amplitude):
        ...
        write = amplitude*np.ones((100,)) # Arbitrarily n = 100 samples, can be any n > 2
        wrote = int32()
